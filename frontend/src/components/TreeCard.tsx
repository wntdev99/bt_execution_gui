 ...
 */
export function TreeCard({ manifest, index, href }: Props) {
  const link = href ?? `/single?tree=${encodeURIComponent(manifest.tree_id)}`;
  const requiredCount = manifest.params.filter((p) => p.required).length;
  return (
    <motion.div
    ...
    >
      <Link
        ...
        className={cn(
          ...
          manifest.dangerous && 'ring-1 ring-warning/30',
        )}
      >
        {manifest.dangerous && (
          <div
        ...
        <div>
          <div className="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-surface-elev text-text-sub transition-colors group-hover:bg-accent-soft group-hover:text-accent">
            <TreeIcon treeId={manifest.tree_id} />
          </div>

          <h3 className="text-title text-text">{manifest.display_name}</h3>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
          <div className="flex items-center gap-3">
            <Spec label="입력" value={`${requiredCount}/${manifest.params.length}`} />
            <Spec label="예상" value={formatEta(manifest.estimated_duration_sec)} />
          </div>
